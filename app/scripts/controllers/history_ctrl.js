app.controller('HistoryCtrl',['$scope', 'jobsService', 'toastr', '$state',
  function ($scope, jobsService, toastr, $state) {
    ...
    function get_my_history_jobs(query){
      $scope.loader = true;
      if(!query) query = ' ';

      jobsService.get_my_history_jobs(query)
        .then(function(res){
          $scope.loader = false;
    ...
    $scope.change_page = function () {
      ...
      if ($scope.morePagesAvailable) {
        ...
        get_my_history_jobs(get_query_string($scope.currentPage, $scope.timeStamp,status));
      }
    ...
    $scope.filter_jobs = function(filter){
      ...
      get_my_history_jobs(get_query_string(undefined,undefined,status))
    };
